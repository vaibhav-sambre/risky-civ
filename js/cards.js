...
import { MAX_HAND_SIZE, CARDS_DRAWN_PER_TURN, EVENT_CARDS } from './config.js';
import { addLog, setEffect } from './state.js';
import { getPlayerTerritories, getTerritory, getNeutralTerritories } from './territories.js';
import { shuffle, randomPick } from './utils.js';
...
export function mustPlayCard(state, playerId) {
...
export function playCard(state, playerId, cardIndex, targetTerritoryId = null) {
    const player = state.players[playerId];
    if (cardIndex < 0 || cardIndex >= player.hand.length) return false;
