const { TROOP_DEPLOY_COST, BASE_DEPLOY_COUNT, STRUCTURES, MIN_GARRISON } = require('./config');
...
function getDeployCount(state, playerId, territoryId) {
    const player = state.players[playerId];
...
function deployTroops(state, territoryId, countOverride = null, free = false) {
...
function moveTroops(state, fromId, toId, count) {
...
module.exports = { getDeployCost, getDeployCount, deployTroops, moveTroops };
